...
async function getAppStateToLogin(loginWithEmail) {
        ...
        try {
                ...
                // is cookie string
                        ...
                        const cookieIsLive = await checkLiveCookie(appState.map(i => i.key + "=" + i.value).join("; "), facebookAccount.userAgent);
                        if (!cookieIsLive)
                                log.warn("LOGIN FACEBOOK", "Cookie live check failed, attempting login anyway...");
                }
        }
        catch (err) {
                spin && spin._stop();
